...
function createTileBag(gameId, cb) {
  ...
  var tiles = [];
  _.forEach(LETTER_FREQUENCIES, function(freq, letter) {
    for (var i = 0; i < freq; i++) {
      tiles.push(letter + i);
    }
  });
  debug('adding tiles: %s', tiles);
  redis.sadd(bagKey, tiles, function(err) {
    cb(err);
  });
}
...
function drawTile(gameId, cb) {
  var bagKey = makeKey(gameId);
  redis.spop(bagKey, function(err, tile) {
    if (err) { return cb(err); }
    var letter = tile ? tile[0] : undefined;
    cb(null, letter);
  });
}
...
function tilesRemaining(gameId, cb) {
  var bagKey = makeKey(gameId);
  redis.scard(bagKey, function(err, remaining) {
    debug('game: %s has: %d tiles remaining', gameId, remaining);
    cb(err, remaining);
  });
}
